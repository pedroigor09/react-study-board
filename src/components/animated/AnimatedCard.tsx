...
function formatExplanation(text: string): string {
  ...
  // Limpar elementos problemáticos de forma mais agressiva
  return formatted
    // Remove parágrafos vazios ou só com espaços
    .replace(/<p class="paragraph">\s*<\/p>/g, '')
    .replace(/<p class="paragraph">[\s\n\r]*<\/p>/g, '')
    // Remove parágrafos que interferem com divs
    ...
    .replace(/(<\/p>)\s*(<p)/g, '$1$2')
    // Remove qualquer tag vazia restante
    .replace(/<[^>]+>\s*<\/[^>]+>/g, '')
